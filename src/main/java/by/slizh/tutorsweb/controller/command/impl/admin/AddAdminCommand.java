...
public class AddAdminCommand implements Command {
    ...
    public Router execute(HttpServletRequest request) throws CommandException {
        ...
        if (user.getEmail().equals(MAIN_ADMIN_EMAIL)) {
            ...
            try {
                userService.makeAdmin(Integer.parseInt(userId));
                return new Router(PagePath.GO_TO_ALL_ADMINS_PAGE, Router.RouteType.REDIRECT);
            } catch (ServiceException e) {
                logger.error("Executing addAdmin command error", e);
                throw new CommandException("Executing addAdmin command error", e);
            }
        }
